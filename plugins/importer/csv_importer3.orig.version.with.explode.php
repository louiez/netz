...
if($filename != ""){ 
	...
	exec("sed --in-place 's/\"//g' ".$filename);
//	exec("sed --in-place \"s/[']//g\" ".$filename);
	$lines =file($filename);
...
$num_csv_fields=count(explode($delimiter,$lines[0]));
...
++++++++++++++++++++++++++++*/
$csv_header=explode($delimiter,$lines[0]);
...
<tr>
<th style="border:solid ; border-width:1px">CSV Fields</th>
...
<th style="border:solid ; border-width:1px">Display Name</th>
...
for ($i=0; $i < 5; $i++){
        ...
        $csvlinedata=explode($delimiter,$lines[$i]);
        echo "<tr>";
        foreach ($csvlinedata as $key=>$v){
                if ($i == 0){
                        echo "<td style=\"border:solid ; border-width:1px ; background-color:lightgrey\"> ".$v."</td>";
                }else{
